...
【GPT 校准详细步骤】(重要, 请严格照做)
* 本机无 TWRP 时用 9008+qdl 读 GPT (黑屏也能用):
  1. 设备进 9008 (关机 音量上+下 插USB; 或 adb reboot edl; 或拆机短接EDL测试点)
  2. 电脑装 QPST 驱动, 设备出现 "Qualcomm HS-USB QDLoader 9008"
  3. 获取 qdl: git clone https://github.com/linux-msm/qdl && cd qdl && make
  4. 读 GPT: ./qdl --storage emmc --print-gpt prog_firehose_ddr.elf
  5. 记下 boot 和 userdata 的 start_sector
* 有 TWRP/root 时:
  - TWRP: `sgdisk --print /dev/block/mmcblk0`; root安卓: `cat /proc/partitions`
* 然后:
  1. 编辑本目录 rawprogram0.xml:
  ...
  2. 换算十六进制: python3 -c "print(hex(起始扇区*512))"
  3. 详细步骤见仓库 docs/gpt-calibration.md
